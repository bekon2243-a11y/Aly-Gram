...
public class UserObject {
    ...
    public static final long ALYGRAM_FOUNDER_ID = 780682804L;
    ...
    public static String getUserName(TLRPC.User user) {
        ...
        String resolvedName = name.length() != 0 || TextUtils.isEmpty(user.phone) ? name : PhoneFormat.getInstance().format("+" + user.phone);
        SharedPreferences preferences = ApplicationLoader.applicationContext.getSharedPreferences("mainconfig", Activity.MODE_PRIVATE);
        boolean founderVisuals = preferences.getBoolean("alygram_founder_visuals", true);
        if (founderVisuals && user.id == ALYGRAM_FOUNDER_ID && !TextUtils.isEmpty(resolvedName)) {
            return resolvedName + " ✅😎";
        }
        return resolvedName;
    }
